...
% Title
\title{\Huge\bfseries\color{mlpurple} Handout 3: Advanced Causal Inference & Production A/B Testing}
...
\section{Handout 3: Advanced Causal Inference \& Production A/B
Testing}\label{handout-3-advanced-causal-inference-production-ab-testing}

\subsection{Week 10: A/B Testing \& Iterative
Improvement}\label{week-10-ab-testing-iterative-improvement}

\textbf{Skill Level: Advanced \textbar{} Mathematical Rigor \textbar{}
...
\textbf{Required Knowledge:} - Advanced probability and statistics -
Causal inference framework (potential outcomes, DAGs) - Production ML
systems experience - Distributed systems concepts

\textbf{Mathematical Notation:} - Y(1), Y(0): Potential outcomes under
treatment and control - ATE: Average Treatment Effect = E{[}Y(1) -
Y(0){]} - CATE: Conditional Average Treatment Effect = E{[}Y(1) - Y(0)
\textbar{} X{]}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Part 1: Causal Inference
Foundations}\label{part-1-causal-inference-foundations}

\subsubsection{Potential Outcomes Framework (Rubin Causal
Model)}\label{potential-outcomes-framework-rubin-causal-model}

\textbf{Fundamental Problem of Causal Inference:} For any unit i, we
observe either Y\_i(1) or Y\_i(0), never both.
...
We cannot observe ITE\_i directly. We estimate population-level effects:
...
\textbf{Key Assumption for Identification:}

\textbf{Unconfoundedness (Conditional Independence):}
...
\textbf{Randomization Breaks Confounding:} In randomized experiments, T
⊥ X, therefore \{Y(1), Y(0)\} ⊥ T.
...
\textbf{Graphical Representation of Causal Relationships:}
...
\textbf{d-Separation Theorem:} Two variables A and B are d-separated by
conditioning set Z if all paths between A and B are blocked by Z.

\textbf{Backdoor Criterion:} A set Z satisfies the backdoor criterion
relative to (T, Y) if: 1. No node in Z is a descendant of T 2. Z blocks
every path from T to Y that contains an arrow into T

\textbf{Adjustment Formula:}
...
\textbf{Meta-Learners for CATE Estimation:}
...
\textbf{X-Learner:} More complex, uses propensity scores and
cross-validation. Performs better with imbalanced treatment assignment.

\textbf{Python Implementation (T-Learner):}
...
\textbf{Interpretation:} Treatment is most effective for young, recent
users.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Part 2: Advanced Statistical
Methods}\label{part-2-advanced-statistical-methods}

\subsubsection{Variance Reduction
Techniques}\label{variance-reduction-techniques}

\textbf{Problem:} Large variance leads to wide confidence intervals and
long experiment duration.

\textbf{Solution:} CUPED (Controlled-Experiment Using Pre-Experiment
Data)

\textbf{Theory:} Use pre-experiment covariate X to reduce variance of
outcome Y.

\textbf{Adjusted Outcome:}

\begin{lstlisting}
Y_adj = Y - θ(X - E[X])
\end{lstlisting}

Where θ is chosen to minimize Var(Y\_adj):

\begin{lstlisting}
θ_optimal = Cov(Y, X) / Var(X)
\end{lstlisting}

\textbf{Key Properties:} - E{[}Y\_adj{]} = E{[}Y{]} (unbiased) -
Var(Y\_adj) = Var(Y) * (1 - ρ\^{}2) where ρ = Corr(Y, X) - If ρ = 0.7,
variance reduced by 51\%, requiring 49\% fewer samples
...
\subsubsection{Stratified Randomization}\label{stratified-randomization}

\textbf{Problem:} Small samples may have imbalance in important
covariates.

\textbf{Solution:} Stratify randomization by key covariates.

\textbf{Algorithm:} 1. Partition users into strata based on covariates
(e.g., new vs returning) 2. Randomize within each stratum 3. Estimate
stratum-specific effects 4. Aggregate using stratum sizes as weights
...
Where w\_s = n\_s / n is stratum weight.
...
\subsection{Part 3: Experimental Design for Complex
Systems}\label{part-3-experimental-design-for-complex-systems}

\subsubsection{Multi-Objective
Optimization}\label{multi-objective-optimization}

\textbf{Problem:} Treatment improves primary metric but degrades
secondary metric.

\textbf{Example:} - Primary: Revenue increases 10\% - Secondary: User
satisfaction decreases 5\%

\textbf{Solution 1: Weighted Utility Function}
...
\textbf{Solution 2: Pareto Optimality} A treatment is Pareto optimal if
no other treatment improves one metric without degrading another.
...
\textbf{Interpretation:} A is dominated by C. E is dominated by B. Must
choose between B, C, D based on business priorities.

\subsubsection{Network Effects and
Interference}\label{network-effects-and-interference}

\textbf{Problem:} SUTVA violated when users interact (social networks,
marketplaces).

\textbf{Example:} - Treatment: Show referral bonus to 50\% of users -
...
Where T\_\{-i\} is treatment assignment of user i's network neighbors.

\textbf{Solutions:}

\textbf{1. Cluster Randomization:} Randomize at community/market level,
not individual level.

\textbf{2. Graph Cluster Randomization:} Partition network into weakly
connected clusters, randomize clusters.

\textbf{3. Two-Stage Randomization:} - Stage 1: Randomize some clusters
to treatment - Stage 2: Within treatment clusters, randomize individuals

\textbf{Python Implementation (Cluster Randomization):}
...
\subsection{Part 4: Production Systems
Architecture}\label{part-4-production-systems-architecture}

\subsubsection{Distributed Experimentation
Platform}\label{distributed-experimentation-platform}

\textbf{Requirements:} 1. Low-latency treatment assignment (less than
10ms) 2. Consistent assignment (same user always sees same variant) 3.
Statistical guardrails with real-time monitoring 4. Multi-tenancy (100s
of simultaneous experiments) 5. Minimal performance overhead

\textbf{Architecture Components:}

\textbf{1. Assignment Service:}
...
\textbf{2. Real-Time Guardrail Monitoring:}
...
\textbf{3. Statistical Power Monitoring:}
...
\subsection{Part 5: Advanced Topics}\label{part-5-advanced-topics}

\subsubsection{Contextual Bandits for
Personalization}\label{contextual-bandits-for-personalization}

\textbf{Problem:} One-size-fits-all treatment suboptimal for
heterogeneous users.

\textbf{Solution:} Learn CATE(x) and assign optimal treatment per user.
...
\tightlist
\item
  \textbf{Causal inference requires strong assumptions}
  ...
  \end{itemize}
\item
  \textbf{Variance reduction techniques can reduce sample size by 50\%+}
  ...
  \end{itemize}
\item
  \textbf{Network effects violate SUTVA}
  ...
  \end{itemize}
\item
  \textbf{Production systems need millisecond-latency assignment}
  ...
  \end{itemize}
\item
  \textbf{Contextual bandits enable personalization}
...
\textbf{Further Reading:} - Imbens \& Rubin: Causal Inference for
Statistics, Social, and Biomedical Sciences - Pearl: Causality: Models,
Reasoning, and Inference - Kohavi, Tang, Xu: Trustworthy Online
Controlled Experiments

\textbf{Next Steps:} - Implement CUPED in your experimentation platform
- Set up real-time guardrail monitoring - Explore contextual bandits for
personalization
