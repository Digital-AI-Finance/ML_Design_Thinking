...
% Title
\title{\Huge\bfseries\color{mlpurple} Finance Applications of ML - Intermediate Handout}
...
\section{Finance Applications of ML - Intermediate
Handout}\label{finance-applications-of-ml---intermediate-handout}

\textbf{Target Audience}: Practitioners with Python knowledge
\textbf{Duration}: 60 minutes reading + coding \textbf{Level}:
Intermediate (implementation focused)
...
\subsection{1. Portfolio Optimization}\label{portfolio-optimization}
...
\subsubsection{Efficient Frontier}\label{efficient-frontier}
...
\subsection{2. Risk Metrics}\label{risk-metrics}
...
\subsubsection{Maximum Drawdown}\label{maximum-drawdown}
...
\subsection{3. Credit Scoring Model}\label{credit-scoring-model}

\subsubsection{Feature Engineering for
Credit}\label{feature-engineering-for-credit}
...
\subsubsection{Credit Scoring with Time-Based
Split}\label{credit-scoring-with-time-based-split}
...
\subsection{4. Trading Strategy
Backtesting}\label{trading-strategy-backtesting}

\subsubsection{Backtest Framework}\label{backtest-framework}
...
\subsubsection{Walk-Forward
Optimization}\label{walk-forward-optimization}
...
\subsection{5. Fraud Detection}\label{fraud-detection}

\subsubsection{Anomaly Detection for
Transactions}\label{anomaly-detection-for-transactions}
...
\subsection{6. Model Validation for
Finance}\label{model-validation-for-finance}

\subsubsection{Regulatory-Compliant
Validation}\label{regulatory-compliant-validation}
...
\subsection{Common Financial Metrics}\label{common-financial-metrics}
...
Information Ratio & \((R_p - R_b) / TE\) & Active return per tracking
error \\
...
\tightlist
\item
  \textbf{Portfolio Optimizer}: Build efficient frontier for ETF
  portfolio
\item
  \textbf{Credit Scorecard}: Develop interpretable credit model
\item
  \textbf{Pairs Trading}: Mean-reversion strategy with cointegration
\item
  \textbf{Fraud Detector}: Real-time anomaly detection system
\end{enumerate}
...
\emph{In finance, a small edge compounded over time can be valuable. But
beware of overfitting - if it looks too good to be true, it probably
is.}
