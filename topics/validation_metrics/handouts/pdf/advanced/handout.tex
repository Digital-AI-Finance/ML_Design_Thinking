...
% Title
\title{\Huge\bfseries\color{mlpurple} Handout 3: Statistical Model Selection & Deployment}
...
\section{Handout 3: Statistical Model Selection \&
Deployment}\label{handout-3-statistical-model-selection-deployment}

\subsection{Week 9 - Advanced Level}\label{week-9---advanced-level}

\subsubsection{Introduction}\label{introduction}

You've mastered multi-metric evaluation and cross-validation. Now we
tackle the hardest problems: How do you select between models when
different metrics favor different choices? How do you quantify
confidence in deployment decisions? How do you monitor model performance
in production and detect when retraining is needed?

\textbf{Target Audience}: Advanced students and practitioners building
production ML systems who need rigorous statistical foundations for
model selection and deployment decision-making.

\textbf{Prerequisites}: Strong Python skills, deep understanding of
classification metrics, experience with sklearn pipelines, familiarity
with statistical hypothesis testing, calculus and linear algebra
background.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Part 1: Advanced Cross-Validation
Strategies}\label{part-1-advanced-cross-validation-strategies}

\subsubsection{Nested Cross-Validation (Unbiased Hyperparameter
Tuning)}\label{nested-cross-validation-unbiased-hyperparameter-tuning}
...
\textbf{Why nested CV?} - Single-level GridSearchCV overestimates
performance (tuned to validation set) - Nested CV gives unbiased
estimate of generalization error - Critical for papers/publications
requiring rigorous evaluation

\subsubsection{Time-Series Walk-Forward
Validation}\label{time-series-walk-forward-validation}
...
\textbf{Critical for}: - Financial time series - User behavior
predictions - Any data with temporal dependencies

\subsubsection{Blocked Cross-Validation (Grouped
Data)}\label{blocked-cross-validation-grouped-data}
...
\subsection{Part 2: Custom Business Metrics and Cost-Sensitive
Learning}\label{part-2-custom-business-metrics-and-cost-sensitive-learning}

\subsubsection{Defining Custom Cost
Functions}\label{defining-custom-cost-functions}
...
\subsubsection{Cost-Sensitive Learning with Sample
Weights}\label{cost-sensitive-learning-with-sample-weights}
...
\subsubsection{Threshold Optimization with Bayesian
Search}\label{threshold-optimization-with-bayesian-search}
...
\subsection{Part 3: Multi-Objective Optimization and Pareto
Frontiers}\label{part-3-multi-objective-optimization-and-pareto-frontiers}

\subsubsection{Pareto Optimal Model
Selection}\label{pareto-optimal-model-selection}
...
\textbf{Interpretation}: Any model on the Pareto frontier is a valid
choice depending on business priorities. Models not on the frontier are
strictly dominated.

\subsubsection{Weighted Multi-Objective
Scoring}\label{weighted-multi-objective-scoring}
...
\subsection{Part 4: Bayesian Model
Comparison}\label{part-4-bayesian-model-comparison}
...
\textbf{Advantages over frequentist tests}: - Directly quantifies
probability one model is better - No p-value misinterpretation -
Incorporates prior knowledge - Handles small sample sizes better

\subsubsection{Bayesian Optimization for
Hyperparameters}\label{bayesian-optimization-for-hyperparameters}
...
\subsection{Part 5: Statistical Power
Analysis}\label{part-5-statistical-power-analysis}
...
\subsection{Part 6: Fairness-Aware Model
Selection}\label{part-6-fairness-aware-model-selection}
...
\subsubsection{Equalized Odds}\label{equalized-odds}
...
\subsection{Part 7: Production Deployment
Framework}\label{part-7-production-deployment-framework}

\subsubsection{Pre-Deployment Checklist
(Code-Based)}\label{pre-deployment-checklist-code-based}
...
\subsubsection{A/B Test Planning}\label{ab-test-planning}
...
\subsection{Part 8: Post-Deployment
Monitoring}\label{part-8-post-deployment-monitoring}

\subsubsection{Concept Drift Detection}\label{concept-drift-detection}
...
Monitoring}\label{performance-degradation-monitoring}
...
\subsection{Part 9: Advanced Topics}\label{part-9-advanced-topics}

\subsubsection{Conformal Prediction (Prediction
Intervals)}\label{conformal-prediction-prediction-intervals}
...
\subsubsection{Adversarial Validation}\label{adversarial-validation}
...
\item
  \textbf{Nested CV for unbiased hyperparameter tuning}: Single-level CV
  overestimates performance.
\item
  \textbf{Custom business metrics}: Align evaluation with actual
  business value, not just ML metrics.
\item
  \textbf{Multi-objective optimization}: Use Pareto frontiers when
  metrics conflict.
\item
  \textbf{Bayesian methods}: Quantify uncertainty in model comparisons
  and hyperparameter search.
\item
  \textbf{Fairness metrics}: Ensure equitable performance across
  demographic groups.
\item
  \textbf{Statistical power}: Calculate required sample sizes before
  experiments.
\item
  \textbf{Production monitoring}: Detect drift and performance
  degradation early.
\item
  \textbf{Conformal prediction}: Provide prediction intervals with
...
\textbf{Advanced Cross-Validation}: - Cawley, G. \& Talbot, N. (2010).
``On Over-fitting in Model Selection and Subsequent Selection Bias in
Performance Evaluation''

\textbf{Cost-Sensitive Learning}: - Elkan, C. (2001). ``The Foundations
of Cost-Sensitive Learning''

\textbf{Multi-Objective Optimization}: - Deb, K. (2001).
``Multi-Objective Optimization Using Evolutionary Algorithms''

\textbf{Bayesian Methods}: - Kruschke, J. (2014). ``Doing Bayesian Data
Analysis''

\textbf{Fairness in ML}: - Barocas, S., Hardt, M., \& Narayanan, A.
(2019). ``Fairness and Machine Learning''

\textbf{Production ML}: - Huyen, C. (2022). ``Designing Machine Learning
Systems''
