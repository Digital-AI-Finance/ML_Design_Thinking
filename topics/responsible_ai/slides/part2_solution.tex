...
% Slide 11a: Real Scenarios - Public Sector
\begin{frame}[t]{Metrics Conflict: Public Sector Scenarios}
\begin{columns}[T]
\column{0.48\textwidth}
\textcolor{mlpurple}{\textbf{University Admissions}}

\small
\textbf{Metrics in tension:}
\begin{itemize}
...
\textcolor{mlorange}{\textbf{Stakeholder conflict:}}\\
Diversity office, faculty, and administration\\
want different fairness definitions

\column{0.48\textwidth}
\textcolor{mlpurple}{\textbf{Criminal Justice}}

\small
\textbf{Recidivism prediction:}
\begin{itemize}
\item DP: Equal risk scores (equal treatment)
\item EO: Equal TPR (catch recidivists)
\item Calibration: Accurate risk (allocation)
\end{itemize}

\textcolor{mlred}{\textbf{Stakes:}}\\
...
\vspace{0.5em}
\begin{tcolorbox}[colback=mllavender!30, colframe=mlpurple]
\textbf{Key Insight:} Public sector decisions require explicit value trade-offs - no metric is universally correct
\end{tcolorbox}

\bottomnote{Context determines fairness priorities - life-altering decisions require transparent metric selection}
\end{frame}

% Slide 11b: Real Scenarios - Private Sector
\begin{frame}[t]{Metrics Conflict: Private Sector Scenarios}
\begin{columns}[T]
\column{0.48\textwidth}
\textcolor{mlpurple}{\textbf{Healthcare Triage}}

\small
\textbf{Resource allocation:}
\begin{itemize}
\item DP: Equal treatment rates per group
\item Individual: Sickest treated first
\item Utilitarian: Maximize QALYs saved
\end{itemize}

\textcolor{mlorange}{\textbf{Ethical frameworks disagree!}}

\vspace{0.5cm}
\textcolor{mlpurple}{\textbf{Employment}}

\textbf{Hiring algorithm:}
\begin{itemize}
\item DP: Equal hiring rates (diversity)
\item EO: Equal callback for qualified (merit)
\item Business: Maximize productivity
\end{itemize}

\textcolor{mlorange}{\textbf{Legal vs business goals}}

\column{0.48\textwidth}
\textcolor{mlpurple}{\textbf{Credit/Lending}}

\small
\textbf{Loan approvals:}
\begin{itemize}
\item DP: Equal approval rates
\item Calibration: Accurate default prediction
\item EO: Equal approval for creditworthy
\end{itemize}
...
\vspace{0.5cm}
\textcolor{mlred}{\Large\textbf{Common Thread:}}\\
\vspace{0.2cm}
...
\bottomnote{Stakeholder value conflicts require domain-specific resolution beyond universal mathematical solutions}
\end{frame}
...
% Slide 13a: Summary - What Metrics Achieve
\begin{frame}[t]{Summary: What Fairness Metrics Achieve}
\begin{columns}[T]
\column{0.48\textwidth}
\textcolor{mlgreen}{\textbf{Detection Success}}

\small
\begin{itemize}
\item DP detected 30\% hidden bias
\item EO revealed 4\% disparity on qualified
\item Calibration showed 1\% accuracy gap
\item All statistically significant
\item Computable in real-time
\end{itemize}

\column{0.48\textwidth}
\textcolor{mlpurple}{\textbf{Available Tools}}

\small
\textbf{20+ metrics in 5 families:}
\begin{itemize}
\item Group fairness (DP, EO, Calibration)
\item Individual fairness (Lipschitz)
\item Causal fairness (path-specific)
...
\vspace{0.3cm}
\textcolor{mlblue}{\textbf{Three Mitigation Stages:}} Pre-processing (data) | In-processing (model) | Post-processing (threshold)

\vspace{0.5em}
\begin{tcolorbox}[colback=mllavender!30, colframe=mlpurple]
\textbf{Success:} Metrics make invisible bias visible and quantifiable
\end{tcolorbox}

\bottomnote{Formalization transforms intuition into measurable criteria - mathematical definitions enable systematic auditing}
\end{frame}

% Slide 13b: Summary - Fundamental Limits
\begin{frame}[t]{Summary: Fundamental Limits of Fairness Metrics}
\begin{columns}[T]
\column{0.48\textwidth}
\textcolor{mlred}{\textbf{Impossibility Results}}

\small
\begin{itemize}
\item Cannot satisfy DP + EO + Calibration
\item Chouldechova: Base rates break compatibility
\item Pearl: 3 independences overconstrain DAG
\item No universal fairness metric exists
\end{itemize}

\column{0.48\textwidth}
\textcolor{mlpurple}{\textbf{What Metrics Miss}}

\small
\begin{itemize}
\item Causation: Why do base rates differ?
\item Values: Which metric is ``right''?
\item Stakeholders: Who decides trade-offs?
\item Context: Domain-specific priorities
\end{itemize}
\end{columns}
...
\vspace{0.5em}
\begin{tcolorbox}[colback=mllavender!30, colframe=mlpurple]
\textbf{Next:} Part 3 explores optimization that makes trade-offs auditable and explicit
\end{tcolorbox}

\bottomnote{Quantification enables optimization despite impossibility - explicit metric selection transforms philosophical debate into engineering}
\end{frame}
